...
export function getAdjacentOpenAreaPositionsFor3D(structure: number[][][], block: number, row: number, column: number): Array<string> {
  const listAdjacentZeroPositions: Array<string> = [];

  try {
    let midRight = -1;
    if (isIndexExistsFor3D(structure, block, row, column + 1)) {
    ...
    let front = -1;
    if (isIndexExistsFor3D(structure, block, row - 1, column)) {
    ...
    let top = -1;
    if (isIndexExistsFor3D(structure, block + 1, row, column)) {
